...
class CustomCanvas(context: Context?) : View(context) {
    override fun onDraw(canvas : Canvas) {
        ...
        canvas.drawCircle((width / 2).toFloat(), (height / 2).toFloat(), (smallerMeasure / 2).toFloat(), brush)
        var path = Path()
        ...
        brush.textSize = 30f
        canvas.drawTextOnPath(resources.getString(R.string.hello_world), path, 0f, 0f, brush)
    }
}